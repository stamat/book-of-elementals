...
| [`<feed-elemental>`](feed.html)                                | [APG Feed](https://www.w3.org/WAI/ARIA/apg/patterns/feed/) — a stream of articles that keeps growing: the indices, the keys and the way out, with the fetching left to you and the scrolling bounded by a budget |
...
| [`<marquee-elemental>`](marquee.html) | No APG pattern, because nothing is operated — a strip that loops, the copies counted against the container rather than guessed at, kept out of the keyboard's way and still under the pointer, and the [stop button](https://www.w3.org/WAI/WCAG22/Understanding/pause-stop-hide.html) every other marquee leaves you to write |
...
| [`<rearrange-elemental>`](rearrange.html) | No APG pattern, because there is none for rearranging a list or a table body by hand — the closest the APG has is its [rearrangeable listbox example](https://www.w3.org/WAI/ARIA/apg/patterns/listbox/examples/listbox-rearrangeable/), and this takes the part that is not the listbox: buttons that move an item, focus that stays on the one that moved, and the live region saying where it landed. Dragging is the option on top, because [WCAG 2.2 SC 2.5.7](https://www.w3.org/WAI/WCAG22/Understanding/dragging-movements.html) asks for it that way round |
