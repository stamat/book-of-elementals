/* book-of-elementals v3.0.0 | https://stamat.github.io/book-of-elementals/ | MIT License */
(() => {
  ...
  function startEdge(slide2, scroller, rtl) {
    return rtl ? scroller.right - slide2.right : slide2.left - scroller.left;
  }
  ...
  var CarouselElemental = class extends ElementBase {
    ...
    readIndex() {
      ...
      const starts = this.slides.map((slide2) => startEdge(slide2.getBoundingClientRect(), box, this.rtl));
    ...
    to(at) {
      ...
      const scroller = this.scroller;
      this.measure(scroller);
      const delta = scrollDelta(startEdge(slide2.getBoundingClientRect(), scroller.getBoundingClientRect(), this.rtl), this.inset, this.rtl);
  ...
  var FeedElemental = class extends ElementBase {
    ...
    escape(after) {
      ...
      const outside2 = Array.from(document.querySelectorAll(FOCUSABLE2)).filter((element) => !this.contains(element) && this.compareDocumentPosition(element) & position);
      if (!after) outside2.reverse();
      for (const element of outside2) {
        element.focus();
  ...
  var SortableTableElemental = class extends ElementBase {
    ...
    sort(index, descending) {
      ...
      for (const at of order) fragment2.append(rows[at]);
      body.append(fragment2);
  ...
  function positionFrom(rect, x, y, options = {}) {
    ...
    const ratio2 = clamp2(along / track, 0, 1);
  ...
  function tiltFrom(rect, pointerX, pointerY, options = {}) {
    ...
    return {
      x: axis === "y" ? 0 : round32(sign * (0.5 - down) * 2 * max),
      y: axis === "x" ? 0 : round32(sign * (across - 0.5) * 2 * max),
      glareX: round32(across * 100),
      glareY: round32(down * 100)
    };
  }
